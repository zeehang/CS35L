...
float printArray[width][height][3];
...
void* threadFunction(void *arg){
	...
	/* for every pixel */
	int firstlimit;
	firstlimit = ((long)arg+1)*divide;
	if((long)arg == (nthreads-1))
	{
		firstlimit += mod;
	}
	for( int px=((long)arg*divide); px<firstlimit; px++ )
	{
			...
			for( int py=0; py<height; py++ )
			{
					...
					for( int i=0; i<3; i++)
					{
							...
							printArray[px][py][i] = scaled_color[i];

					}
					// printArray[px][py][0] = scaled_color[0];
					// printArray[px][py][1] = scaled_color[1];
					// printArray[px][py][2] = scaled_color[2];
    ...
    */
    ...
    printf( "# Rendering scene with %d spheres and %d lights\n",
  	...
  	for(t = 0; t < nthreads; t++)
		...
		//this is where you print stuff out
		/*
		printf( "%.0f %.0f %.0f\n",
		scaled_color[0], scaled_color[1], scaled_color[2] );
		//the above is the inner loop
...
*/
		for(int i = 0; i < width; i++)
		{
			...
			{
				printf("%.0f %.0f %.0f\n",printArray[i][j][0],printArray[i][j][1],printArray[i][j][2]);
			}
			printf("\n");
		}
    free_scene( &scene );
